...
public class ExcelUtils {
    ...
    private static <T> void setFieldValue(T t, Field field, JSONObject obj, StringBuilder uniqueBuilder, List<String> errMsgList) {
        ...
        //正则匹配,不符合要求的导入信息枚举类显示
        RegexPatternEnum pattern = annotation.pattern();
        if (!Pattern.matches(pattern.getRegex(), val)) {
            errMsgList.add(String.format("[%s]的值不符合要求(当前值为%s),修改建议%s", cname, val, pattern.getMessage()));
            return;
        }
        ...
        try {
            if ("String".equalsIgnoreCase(fieldClassName)) {
                ...
                field.set(t, new BigDecimal(String.valueOf(val)));
    ...
    private static ExcelClassField getExcelClassField(Field field) {
        ...
        cf.setMust(annotation.required());
        return cf;
    }
    ...
    private static LinkedHashMap<String, String> getKvMap(String kv) {
        ...
        for (String each : kvs) {
            ...
            if (eachKv.length != 2) {
                // key可以去负值的改动
                int i = getString(each).lastIndexOf("-");
                k = getString(each).substring(0, i);
                v = getString(each).substring(i + 1);
            } else {
                k = eachKv[0];
                v = eachKv[1];
            }
    ...
    private static <T> List<List<Object>> getSheetData(List<T> list) {
        ...
        for (ExcelClassField each : excelClassFieldList) {
            ...
            if (each.getMust()) {
                headList.add(each.getName() + "MUST");
            } else {
                headList.add(each.getName());
            }
    ...
    private static void export(HttpServletResponse response, File file, String fileName,
        ...
        for (Entry<String, List<List<Object>>> entry : entries) {
            ...
            //设置重点信息背景
            CellStyle importStyle = book.createCellStyle();
            importStyle.setFillForegroundColor(IndexedColors.GREY_80_PERCENT.index);
            importStyle.setFillPattern(FillPatternType.SOLID_FOREGROUND);
            importStyle.setAlignment(HorizontalAlignment.CENTER);
            Font font = book.createFont();
            font.setColor(IndexedColors.RED.index); // 设置字体颜色为红色
            importStyle.setFont(font);
            importStyle.setFillForegroundColor(IndexedColors.GREY_25_PERCENT.index);
            importStyle.setFillPattern(FillPatternType.SOLID_FOREGROUND);
            // 设置表身背景色（默认色）
                ...
                for (int j = 0; j < rowList.size(); j++) {
                    ...
                    if (o instanceof URL) {
                        ...
                        if (i == 0) {
                            String s = o.toString();
                            if (s.contains("MUST")) {
                                String value = s.substring(0, s.length() - 4);
                                v = setCellValue(cell, value, importStyle);
                            } else {
    ...
    private static int setCellValue(Cell cell, Object o, CellStyle style) {
        ...
        if (o instanceof String) {
            ...
            if (isNumeric(s) && s.length() < 8 && s.length() > 0) {
    ...
    private static boolean isNumeric(String str) {
        if (Objects.nonNull(str) && ".00".equals(str)) {
            return true;
        }
     ...
     * @return
     */
    public static ExcelReader checkExcel(MultipartFile file) {
        if (file.isEmpty()) {
            throw new ServiceException("传入的文件为空");
        }
        String fileName = file.getOriginalFilename();
        String fileType = fileName.substring(fileName.lastIndexOf(".") + 1).toLowerCase();
        if (!"xls".equals(fileType) && !"xlsx".equals(fileType)) {
            throw new ServiceException("文件格式不对，只支持xls,xlsx");
        }
        ExcelReader reader = null;
        try {
            reader = ExcelUtil.getReader(file.getInputStream());
        } catch (IOException e) {
            throw new ServiceException("解析excel文件数据异常！异常信息：" + e.getMessage());
        }
    ...
    public static List<?> importExcel(MultipartFile file, Class<?> clazz) {
        if (file == null || file.isEmpty()) {
            throw new ServiceException("没有文件或者文件内部为空");
        }
            ...
            switch (cellType) {
                case NUMERIC:
                    if (DateUtil.isCellDateFormatted(cell)) {
                        ...
                        result = cn.hutool.core.date.DateUtil.formatDateTime(theDate);
                    } else {
    ...
    public static Boolean isCorrectFile(MultipartFile file) {
        ...
        if (fileName.lastIndexOf(".") == -1 || !(XLS.equals(suffix) || XLSX.equals(suffix))) {
